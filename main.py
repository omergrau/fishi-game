...
from specialities.ShieldBubble import ShieldBubble
...
class game():
    ...
    def update_shield_bubble(self):
        now = pygame.time.get_ticks()
        player1 = self.players.sprites()[0]
        if self.shield_bubble:
            if pygame.sprite.collide_mask(player1, self.shield_bubble):
            ...
            if pygame.sprite.collide_mask(player1, fish_i):
                if player1.size > fish_i.size:
                    ...
                    if player1.shield:
                        ...
                        player1.shield_cooldown_start = pygame.time.get_ticks()
                    elif player1.shield_cooldown_start is None:
                        self.game_mode = "game over"

    def draw_shield_Bubble(self):
        if self.shield_bubble is not None:
            self.screen.blit(self.shield_bubble.image, self.shield_bubble.rect)
        elif self.players.sprites()[0].shield:
            self.screen.blit(self.players.sprites()[0].shield.image, self.players.sprites()[0].shield.rect)

    def draw(self):  # draw the game screen and the player and enemies
        self.fishlist.draw(self.screen)
        self.draw_shield_Bubble()
